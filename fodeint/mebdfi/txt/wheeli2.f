...
C     01/04/2000
C*************************************************************************
C     DRIVER FOR MEBDFI ON THE INDEX 2 WHEEL PROBLEM
C************************************************************************
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C...
C...  PARAMETERS FOR MEBDFI
C...
      PARAMETER (ND=17,LWORK=(41+3*ND)*ND+3,LIWORK=ND+14)
      DIMENSION Y(ND),WORK(LWORK),IWORK(LIWORK),TRUE(ND),MBND(4),
     +     ERROR(ND),RPAR(2),IPAR(2),ATOL(ND),RTOL(ND),
     + YPRIME(ND)          
      CHARACTER*(*) SOLVER, PROBLEM
      PARAMETER (SOLVER  = ' MEBDFI',PROBLEM = 'WHEEL SET',
     +     IND=2)
C...
      EXTERNAL PDERV, RESID
C      
      open(6,file='wheeli2.out')
      rewind(6)
      open(8,file='wheelaci2.out')
      rewind(8)
C
...
C...  REQUIRED TOLERANCE
         ...
         WRITE(6,*) 'RTOL =' ,RTOL(1),' ',RTOL(17)
         WRITE(6,*) 'ATOL =' ,ATOL(1),' ',ATOL(17)
C...  INITIAL VALUES         
...
C...  ISTICS FOR MEBDFI AND GO ON TO THE NEXT RUN 
C...        
...
C     POSITION VARIABLES X, Y, Z, THETA, PHI
      Y( 1) =   0.14941D-02
      Y( 2) =   0.40089D-06
      Y( 3) =   0.11241D-05
      Y( 4) =   -.28573D-03
...
C     VELOCITIES
      DO 5 I=6,10
        Y(I)=0.0D0
 5    CONTINUE
C     DYNAMIC VARIABLE BETA
      Y(11) =   0.0d0
C     LAGRANGE MULTIPLIERS (SCALED BY*1.E-4)
      Y(16) =   -.83593d-02
      Y(17) =   -.74144d-02
C     CONTACT COORDINATES PSIL, XIL, PSIR, XIR
c     old values
c     Y(14) =   0.75634d-05
c     Y(15) =   0.14906d+00
c     Y(12) =   -.74122d-05
...
c     YPRIME( 8) =  .79669d-1
c     YPRIME( 9) = -.55321d+1
c     YPRIME(10) = -.34985d+0
c     YPRIME(11) = -.21308d+1
c     new values
c     (determined numerically by Jacques de Swart, 23 October 1997)
      yprime( 6)= -1.975258894011285d0
      yprime( 7)= -1.0898297102811276d-03
      yprime( 8)=  7.8855083626142589d-02
      yprime( 9)= -5.533362821731549d0
      yprime(10)= -0.3487021489546511d0
      yprime(11)= -2.132968724380927d0
      DO 7 I=12,17
        YPRIME(I)=0.0D0
  7   CONTINUE
