...
C     03/04/2000
C*************************************************************************
C     DRIVER FOR MEBDFI ON THE INDEX 1 TWO BIT ADDING PROBLEM
C************************************************************************
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C...
C...  PARAMETERS FOR MEBDFI
C... 
      PARAMETER (ND=350,LWORK=(38+3*ND)*ND+3,LIWORK=ND+14)   
      DIMENSION Y(ND),WORK(LWORK),IWORK(LIWORK),TRUE(ND),
     +     ERROR(ND),RPAR(2),IPAR(2),YPRIME(ND),MBND(4)          
      CHARACTER*(*) SOLVER, PROBLEM
      PARAMETER (SOLVER  = ' MEBDFI',PROBLEM = 'TWO BIT ADDING',
     +     IND=1)
C...
      EXTERNAL PDERV, RESID
C...
      open(6,file='tbai.res')
      rewind(6)
      open(8,file='tbaaci.res')     
      rewind(8)         
C      
...
C...         
 ...
 30   CONTINUE
      ...
      END
c-----------------------------------------------------------------------
      SUBROUTINE RESID(N,T,Y,DF,YPRIME,IPAR,RPAR,IERR)
      DOUBLE PRECISION T, Y(N), YPRIME(N), DF(N),RPAR(2)
      INTEGER N, IPAR(2)

      CALL F(N,T,Y,DF,IPAR,RPAR,IERR)
C
      DO I=1, 175
         DF(I) =  YPRIME(I) - DF(I)
      ENDDO
...
C            
      DO I=1,NEQN
         SKIPI =  i.ne.49+175 .and. i.ne.130+175 .and. i.ne.148+175
