...
C     4/02/2000
C*************************************************************************
C     DRIVER FOR MEBDFDAE ON THE INDEX 1 PENDULUM  PROBLEM
C************************************************************************* 
     ...
     +     ERROR(ND),MASBND(4)
      ...
      PARAMETER (SOLVER  = ' MEBDFDAE',PROBLEM = 'PENDULUM',IND=1)
C     
      EXTERNAL F,PDERV,MAS
C
      open(6,file='pendul1.res')
      rewind(6)
      open(8,file='pendul1ac.res')
      rewind(8)
C
...
C...
 220     CONTINUE      
     ...
     +        WORK,LIWORK,IWORK,MBND,masbnd,MAXDER,ITOL,RTOL,
     +        ATOL,RPAR,IPAR,F,PDERV,MAS,IER)
         IF (INDEX.EQ.1) THEN
            INDEX=0
            GOTO 220
...
C...
         ...
         write(6,1289)
 1289    format(1x,'following are conservation laws that should hold')
...
C-------------------------------------------------------------------------
      ...
      DOUBLE PRECISION AM(N,N),T,IPAR(*),RPAR(*)
      INTEGER N      
C
      am(1,1)=1.0D+0
      am(1,2)=0.0D+0
