...
C     4/02/2000
C*************************************************************************
C     DRIVER FOR MEBDFDAE ON THE INDEX 2 PENDULUM  PROBLEM
C************************************************************************* 
     ...
     +     ERROR(ND),MASBND(4)
      ...
      PARAMETER (SOLVER  = ' MEBDFDAE',PROBLEM = 'PENDULUM',IND=2)
C     
      EXTERNAL F,PDERV,MAS
C
      open(6,file='pendul2.res')
      rewind(6)
C
...
C...
         ...
         write(6,1289)
 1289    format(1x,'conservation laws that should be satisfied')
...
C-------------------------------------------------------------------------
      ...
      DOUBLE PRECISION AM(M,N),T,IPAR(*),RPAR(*)
      INTEGER N      
C
      am(1,1)=1.0D+0
      am(1,2)=0.0D+0
