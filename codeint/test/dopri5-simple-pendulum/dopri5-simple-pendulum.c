...
/*
      ...
      DOPRI5_RETVAL ret = dopri5_basic(stack, y, &simple_1d, neq, x0, xf, dx); 
      if (ret != SUCCESS) {
      fprintf(stderr, "error encountered in dopri5_basic\n");
      return -1;
      }
    ...
    double *stack = create_stack(x0, xf, dx, neq);
    DOPRI5_RETVAL ret = dopri5_basic(stack, y, &simple_2d, neq, x0, xf, dx); 
    if (ret != SUCCESS) {
    fprintf(stderr, "error encountered in dopri5_basic\n");
        return -1;
    }
...
*/
    ...
    DOPRI5_RETVAL ret = dopri5_basic(stack, q, &simple_pendulum, neq, t0, tf, dt); 
    if (ret != SUCCESS) {
        fprintf(stderr, "error encountered in dopri5_basic\n");
        return -1;
    }
